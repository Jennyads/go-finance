...
const getAccountsGraph = `-- name: GetAccountsGraph :many
...
func (q *Queries) GetAccountsGraph(ctx context.Context, arg GetAccountsGraphParams) ([]int64, error) {
	rows, err := q.query(ctx, q.getAccountsGraphStmt, getAccountsGraph, arg.UserID, arg.Type)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const getAccountsReports = `-- name: GetAccountsReports :many
...
func (q *Queries) GetAccountsReports(ctx context.Context, arg GetAccountsReportsParams) ([]int64, error) {
	rows, err := q.query(ctx, q.getAccountsReportsStmt, getAccountsReports, arg.UserID, arg.Type)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		var sum_value int64
		if err := rows.Scan(&sum_value); err != nil {
			return nil, err
		}
		items = append(items, sum_value)
	}
	if err := rows.Close(); err != nil {
		return nil, err
	}
...
const updateAccounts = `-- name: UpdateAccounts :many
...
func (q *Queries) UpdateAccounts(ctx context.Context, arg UpdateAccountsParams) ([]Account, error) {
	rows, err := q.query(ctx, q.updateAccountsStmt, updateAccounts,
		arg.ID,
		arg.Title,
		arg.Description,
		arg.Value,
